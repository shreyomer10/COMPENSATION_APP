...
@Composable
fun DraftApplication(navController: NavController, guard: String?) {
    val gson = Gson()
    ...
    var Forms by remember {
        mutableStateOf<List<FormData>>(emptyList())
    }
    viewModel.getAllForms {
        Forms=it
    }
    Log.d("DRAFT APP", "DraftApplication: $gguard")
    Column {
        ...
        LazyColumn {
            if (Forms.isNotEmpty()) {
                items(Forms) { form ->
                    ApplicationItemDraft(form=form, guard = guard,navController = navController)
                }
            } else {
                item {
...
@Composable
fun ApplicationItemDraft(navController: NavController,guard: String?, form: FormData) {
    val gson = Gson()
    ...
    Card(
        ...
        Column(
            ...
            Button(
                onClick = {
                    navController.navigate(NavigationScreens.EditDraftScreen.name +"/$guard"+ "/$encodedForm")
                },
